...
r = sr.Recognizer()
files = sorted(glob.glob(r"c:\Users\PC\Desktop\Waze German Voice\tools\spongebob_samples_more\00_*.mp3"))
if files:
    src = files[0]
    data, sr_rate = sf.read(src)
    dur = len(data) / sr_rate
    print(f"Scanning {src} ({dur:.1f}s)...")
    temp_wav = r"c:\Users\PC\Desktop\Waze German Voice\tools\temp_sb.wav"
    t = 0.0
    while t + 3.0 <= dur:
